...
| **P2** ✅ | Contract-verify delegated & MCP resource-server tokens — `internal/agentauth` statically checks `aud` binding (RFC 8707/9728), the delegation `act` claim and chain shape (RFC 8693), scope minimization, and expiry. `keyway agent inspect`; covers MCP-01/02, DEL-01/02, SCOPE-01/02 (agent domain 0 → 40%). | contract discovery + diff |
