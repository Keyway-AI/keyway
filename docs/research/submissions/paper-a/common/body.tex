...
Whether a bearer token is accepted is decided not by code but by
\emph{configuration}---the issuers a verifier trusts, the audiences it binds to,
the algorithms it allows, the claims it requires---an implicit \emph{authorization
contract} that is rarely written down and, when it drifts, causes outages or silent
over-acceptance. We argue this contract should be a first-class, measurable object,
and take a concrete first step: a method to derive it automatically from deployment
configuration, and a preliminary measurement of its hygiene across 428 public
repositories. We quantify RFC-defined weaknesses---unbound audiences, absent
algorithm pinning, missing required claims---with Wilson confidence intervals; and
because a measurement is only as trustworthy as its instrument, we validate
extraction with a kind-aware oracle for recall (issuers 89.6\%, audiences 99.0\%)
and a deliberately non-circular negative control for precision, reporting what each
can and cannot establish. We release the instrument, the corpus manifest, and the
analysis, and set out the research agenda---scale, drift, and the static/runtime
frontier---that a full study will complete.
...
contract}. It is emergent (spread across service meshes, proxies, identity
providers, and library defaults), almost never centralised, and it \emph{drifts}:
a rotated key, a widened audience, a dropped claim turns valid users away (an
outage) or admits tokens that should be refused (a silent regression). And as
autonomous agents begin carrying OAuth/MCP tokens on a user's behalf, the same
contract now governs a faster-moving, higher-stakes surface.

Despite being where auth correctness actually lives, this contract is essentially
\emph{unmeasured}. Two classes of tooling exist and neither answers ``what does the
...
issuer; gateways and IdPs \emph{are} the artifact under test, not a test of it. We
argue the authorization contract should be a first-class, measurable object, and
this paper is the first step toward measuring it.

\noindent\textbf{Contributions.}
(i) A framing---the authorization contract as a derivable, versioned, measurable
artifact---and a method to derive it from heterogeneous deployment configuration
with per-field provenance (\S\ref{sec:method}).
(ii) A preliminary measurement of the prevalence of concrete, RFC-cited weaknesses
across 428 public repositories, with confidence intervals (\S\ref{sec:results}).
(iii) A validation methodology candid about its own limits---a kind-aware oracle
for recall and a non-circular negative control for precision, each with its
failure mode stated (\S\ref{sec:validation}); we contend this candor is what a
security measurement should look like.
...
\noindent This is deliberately a \emph{vision-and-first-step} paper: we contribute
the framing, a validated instrument, and an honest baseline, and set the agenda the
community (and we) should pursue; \S\ref{sec:limitations} is candid about what the
